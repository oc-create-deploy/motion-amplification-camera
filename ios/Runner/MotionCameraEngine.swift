...
enum FrequencyEstimator {
  static func dominantFrequency(samples: [Float], timestamps: [Double], lowerHz: Double, upperHz: Double) -> Double {
    ...
    guard let dft = vDSP.DFT(count: count, direction: .forward, transformType: .realToComplex, ofType: Float.self) else { return 0 }; var real = [Float](repeating: 0, count: count / 2), imag = real; dft.transform(input: signal, outputReal: &real, outputImaginary: &imag); var power = [Float](repeating: 0, count: count / 2); for index in power.indices { power[index] = real[index] * real[index] + imag[index] * imag[index] }
